...
const panels: Panel[] = [
  ...
  },
  {
    ...
    desc: "For any trace, preview the literal MCP payload an agent receives alongside the never-captured list before you approve a connection. You control where the console and trace store run.",
    proof: "GET /session/{id}/privacy-posture",
  },
  {
    icon: ShieldCheck,
    title: "Edit the scrub policy live",
    desc: "Add custom redaction patterns and dropped fields from the dashboard, with a live preview. Edits can only tighten the boundary, never loosen the built-in PII rules. Tests enforce that invariant.",
  ...
  return (
    <Section id="console" className="border-b border-line">
      <SectionHeader
        ...
        body="The self-hosted console groups traces by what actually broke: same route, same failure, same structure. Forty reports of one bug become one card and one decision. It moves through the pipeline as your CI reports back, and it governs which agent gets which evidence. Each panel points at the open-source code that implements it."
      />
