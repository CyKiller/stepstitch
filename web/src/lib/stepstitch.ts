    ...
    warnings: [
      {
        ...
        detail: "Route /accounts/:id/transfer needs a seeded account fixture.",
        step_index: 1,
      },
    ],
    signals: { steps: 6, interactive: 4, stable_selectors: 4 },
  },
  privacy: {
    scrub_status: "clean",
...
test('repro trc_9f4c1ae2: transfer review returns 500', async ({ page }) => {
  // Seed fixture for the templated route /accounts/:id/transfer
  ...
  const failing = page.waitForResponse(
    (r) => r.url().includes('/api/accounts/') && r.status() === 500,
  );
  await page.locator('[data-testid=review-transfer]').click();

  // Currently red: the API returns 500. Fix should make this green.
  const res = await failing;
  expect(res.status(), 'transfer review must not 500').toBeLessThan(500);
});`,
};
