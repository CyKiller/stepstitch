...
- `/api/contact` relays the pilot form to `CONTACT_WEBHOOK_URL`. The variable is
  **required in production**: without it the route refuses with a 503 (and the form says
  the message did not send) rather than showing a false success over a discarded
  submission. The post-deploy canary (`.github/workflows/verify-deploy.yml`) POSTs
  `{"canary": true}` and fails the deploy check until the relay delivers for real.
