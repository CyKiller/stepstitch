...
1. **Kill capture (org-wide):** flip the tenant kill-switch config flag, and/or call
   `tracker.disable()` in the shipped bundle. New traces stop immediately.
...
5. **Recover:** rotate ingestion credentials, purge affected trace bodies (the deletion
   itself is audit-logged), redeploy a fresh signed SDK build (verify SRI hash).

## Supporting controls

- **Kill switch:** tenant config flag + SDK `disable()`.
- **Audit trail:** every operator read of a trace writes a tamper-evident access event.
- **Split retention:** short-lived trace bodies; 5-year audit records.
- **Supply chain:** SDK ships with an SRI hash, signed artifact, and SBOM so the tenant
  pins an exact verified build; version + build hash are embedded in trace metadata for
  forensics.
