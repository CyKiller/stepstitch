...
| Suite | Tests | Command |
|---|---|---|
| Service (compiler, router, privacy, connectors) | **938** | `PYTHONPATH=service pytest service/tests/` |
...
| **FixProof trust chain (cryptographic)** — `require_signature` verifies a real Ed25519 signature (the `cryptography` library; RFC 8032 vectors pinned) against `trusted_keys` the policy names — presence is never authenticity, and trust anchors are validated prime-order (a small-order key, against which one forged signature verifies every message, is an unusable policy); `require_bindings` refuses proofs missing any load-bearing binding; `allowed_verifier_identities` allowlists who may verify; `stepstitch proof keygen` creates the anchor; the unconfigured template refuses to run (exit 2); the six trust-audit attacks are each a permanent refusal test; the live loop signs with an ephemeral run key and proves tampered AND differently-signed copies refused | `_ed25519.py`, `fixproof.py`, `host/signing.py`, `cli.py` | `test_ed25519.py`, `test_fixproof_adversarial.py`, `test_proof_cli.py`, `test_fixproof_endpoint.py`, CI job `tiny-transfer-live` (steps 10–13) |
| **FixProof merge protocol (proof-only commit)** — the executable customer flow the second audit demanded: commit A is the tested code the signed proof names, commit B (the PR head) adds only `fixproof.json`; `stepstitch proof gate` enforces one parent + only-proof diff + subject == HEAD^ with read-only git queries; the gate workflow runs from the protected base via `pull_request_target` (a PR can weaken neither the gate nor the policy; the head is fetched as data, never executed) and pins actions + verifier version; the repro workflow reports resolved `base_commit`/`fixed_commit` from its own checkouts; proven in real temporary git repositories — happy path, code-after-proof, stowaway file, self-referential proof, merge head, forged and small-order keys | `cli.py` (`proof gate`), `github_bridge/workflow.py` | `test_proof_gate.py`, `test_github_content.py`, `test_fixproof_adversarial.py` |
