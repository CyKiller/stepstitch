...
- **Acceptance (green):** `test_mcp_surface.py` — `test_no_destructive_mcp_tool`, `test_tool_definitions_have_schemas`, and `test_dispatch_drives_service_sanitized_and_audited` (E2E: tool call → real service → sanitized, no NPI, audited). **86 service tests green.**
